...
class Transformer(nn.Module):
    def __init__(
        self, d_model: int, n_heads: int, n_vocab: int, n_layers: int, n_ctx: int
    ) -> None:
        super().__init__()
        self.d_model = d_model
        self.n_heads = n_heads
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        pos = torch.arange(0, N, dtype=torch.long)
...
def main():
    ...
    repeated_sequence = torch.randint(0, N_VOCAB, (B, repeat_size), dtype=torch.long)
