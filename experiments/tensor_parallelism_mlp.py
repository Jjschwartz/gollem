...
class MLP(nn.Module):
    def __init__(self, d_model: int, d_mlp: int, devices: list[torch.device]) -> None:
        ...
        self.w_block_size = self.d_mlp // n_devices

        w1_blocks = []
        w2_blocks = []
        for d in self.devices:
            w1 = nn.Linear(d_model, self.w_block_size, device=d)
            w1_blocks.append(w1)
            w2 = nn.Linear(self.w_block_size, d_model, device=d)
            ...
            # which is the w_block_size by default, so we we need to manually set this
            # to use 1/d_mlp
            nn.init.normal_(w2.weight, mean=1 / d_mlp, std=1 / d_mlp)
            w2.bias.data.zero_()
...
def compare_models_pairwise_across_devices(
    x: torch.Tensor, d_model: int, d_mlp: int, devices: list[torch.device]
):
    model_single = MLP(d_model=d_model, d_mlp=d_mlp, devices=devices[:1])
    y = model_single(x)

    for n_devices in [1, 2, 4, 8]:
        if n_devices > len(devices):
            break
        model_n = MLP(d_model=d_model, d_mlp=d_mlp, devices=devices[:n_devices])

        if n_devices == 1:
            model_n.load_state_dict(model_single.state_dict(), strict=True)
        else:
            # need to manually split the state dict and load it
            state_dict = model_single.state_dict()
            model_n.load_state_dict(model_single.state_dict(), strict=False)

        y_n = model_n(x)
        all_close = torch.allclose(y, y_n)
